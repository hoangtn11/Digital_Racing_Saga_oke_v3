...
class laneDetector(threading.Thread):
    ...
    def apply_canny_filter(self):
        ...
        lower1, upper1 = np.array([0 , 0, 222]), np.array([179, 28, 255])
        ...
        two_thirds_height = (1.7 * height) // 3
        two_thirds_height = int(height - two_thirds_height)
        img_thresholded = mask_1.copy()
        ...
        filtered_edges = np.zeros_like(img_thresholded)
    ...
    def run(self):
        while not gs.exit_signal:
            ...
            put_to_queue_no_wait_no_block(self.mask, gs.mask_img)
            put_to_queue_no_wait_no_block(self.mask, gs.dc_images)
